...
def main(threshold=50):
    ...
    for text in open('./data/cornell_movie-dialogs_corpus/movie_conversations.txt', 'r', encoding='utf-8'):
        ...
        if len(conv_list) >= 3:
            conv_id_list.append((conv_list[0], conv_list[1], conv_list[2]))
    print(len(conv_id_list))
    # remove html tags and modify the sentences containing  '...'
    tag_pattern = r"<[^>]*?>"
    ...
    for tup in conv_id_list[:]:
        if text_dic.get(tup[0], False) and text_dic.get(tup[1], False) and text_dic.get(tup[2],False):
            # remove html tags
            text_dic[tup[0]] = tag.sub("", text_dic[tup[0]])
            text_dic[tup[1]] = tag.sub("", text_dic[tup[1]])
            text_dic[tup[2]] = tag.sub("", text_dic[tup[2]])

            # replace the sentences containing '...' with '#'
            ...
            if is_english(text_dic[tup[0]] + text_dic[tup[1]] + text_dic[tup[2]]):
                ...
                cmnts2.append([unicodedata.normalize('NFKC', word.lower())
                                  for word in word_tokenize(text_dic[tup[2]])])
            else:
                del text_dic[tup[0]]
                del text_dic[tup[1]]
                del text_dic[tup[2]]
    ...
    with open('data/three_corpus.txt', 'w', encoding='utf-8') as f:
        for i in range(len(posts)):
            ...
            cmnt2 = set([corpus.dfs[corpus.token2id[word]] for word in cmnts2[i]])
            if len(post & cut_freq) == 0 and len(cmnt & cut_freq) == 0 and len(cmnt2 & cut_freq) == 0:
                f.write(text_dic[conv_id_list[i][0]] + '\t' + text_dic[conv_id_list[i][1]]\
                        + '\t' + text_dic[conv_id_list[i][2]] + '\n')
